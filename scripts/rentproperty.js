// Function to initialize the rent property form

console.log('Initializing the rent property form...');

function initializeRentPropertyForm() {
    ...
    document.getElementById('rentproperty-form').addEventListener('submit', function (event) {
        event.preventDefault(); // Prevent the default form submission
        console.log("Form submission started.");

        // Validate required fields
        const requiredFields = document.querySelectorAll('.rentproperty_input[required]');
        ...
        if (clientId) {
         ...
         // Check photo uploads again before sending the data
        if (photoFiles.length < 3) {
            alert('Please upload at least 3 photos.');
            return;
        }
        ...
        fetch('http://localhost:3000/api/rentproperty', {
            method: 'POST',
            body: formData
        })
        .then(response => {
    ...
    console.log('executed fully the rent property form function...');
}

initializeRentPropertyForm();
