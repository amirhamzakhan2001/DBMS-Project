console.log('for_rent.js start executing');

function initForRentSearch() {
    console.log('for_rent.js function start executing');

    const searchButton = document.getElementById("search-button");
    const resultsContainer = document.createElement("div");
    const detailedViewContainer = document.createElement("div");

    resultsContainer.id = "rent_result_results-container"; // Set ID for results container
    detailedViewContainer.id = "rent_result_detailed-view-container"; // Set ID for detailed view container
    ...
    function getSearchCriteria() {
        ...
        const monthly_price = document.getElementById("monthly_price").value || null; // Allow empty

        return { locality, property_type, number_of_bedrooms, monthly_price };
    }

    async function fetchProperties({ locality, property_type, number_of_bedrooms, monthly_price }) {
        try {
            console.log('Sending request to fetch properties...');
            resultsContainer.innerHTML = '<p>Loading...</p>'; // Show loading
            const response = await fetch("http://localhost:3000/api/forrentproperties", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                },
                body: JSON.stringify({ locality, property_type, number_of_bedrooms, monthly_price }),
            });
    ...
    function displayResults(properties) {
        ...
        properties.slice(startIndex, endIndex).forEach((property, index) => {
            ...
            startAutoSlideshow(propertyContainer.querySelector('.rent_result_image-slideshow'), imagesHtml);
    ...
    function createPropertyContainer(property, photoArray) {
        ...
        propertyContainer.classList.add("rent_result_property-result");

        // Create image slideshow container
        const imageContainer = document.createElement('div');
        imageContainer.classList.add('rent_result_image-slideshow');
        propertyContainer.appendChild(imageContainer);

        // Property details
        const detailsContainer = document.createElement("div");
        detailsContainer.classList.add("rent_result_property-details");
        detailsContainer.innerHTML = `
            <h3 class="rent_result_property-title" data-id="${property.id}">${property.property_title}</h3>
        ...
        const titleElement = detailsContainer.querySelector(".rent_result_property-title");
    ...
    function startAutoSlideshow(container, images) {
        ...
        images.forEach(imageUrl => {
            ...
            img.src = imageUrl;
            img.classList.add('rent_result_image-slideshow');
        ...
        const imageElements = container.querySelectorAll('.rent_result_image-slideshow');
    ...
    function createImageGrid(images, title) {
        const imageContainer = document.createElement("div");
        imageContainer.classList.add("rent_result_image-grid");
    
        // Create a wrapper for the images to arrange them in a grid
        const gridWrapper = document.createElement("div");
        gridWrapper.classList.add("rent_result_grid-wrapper");
    
        images.forEach((img, index) => {
            const imgElement = document.createElement("img");
            imgElement.src = `http://localhost:3000/uploads/photos/${img}`; // Adjust the path
            imgElement.alt = title;
            imgElement.classList.add("rent_result_property-image");
    
            // Create a container for each image
            const imageWrapper = document.createElement("div");
            imageWrapper.classList.add("rent_detailed_image-container");
    ...
    function showDetailedView(property, photoArray) {
        console.log('Showing detailed view for rent property:', property);
    
        // Create and append the overlay
        const overlay = document.createElement('div');
        overlay.className = 'rent_result_body-overlay';
        document.body.appendChild(overlay);
    
        // Prevent background scrolling
        document.body.classList.add('rent_resultoverlayno-scroll');
    
        // Create detailed view container
        const detailedViewContainer = document.createElement('div');
        detailedViewContainer.className = 'rent_result_detailed-view';
        ...
        detailedViewContainer.innerHTML = `
            <button class="rent_result_close-button">×</button>
            <div class="rent_result_image-grid-container">${imageGrid.outerHTML}</div>
            <div class="rent_result_info-container">
                <div class="rent_result_info-left">
                    <h2 class="rent_result_property-title">${property.property_title}</h2>
                    <p><strong>Property Type:</strong> ${property.property_type}</p>
                    <p><strong>Monthly Rent:</strong> ₹${property.monthly_price}</p>
                ...
                <div class="rent_result_info-right">
        ...
        const closeButton = detailedViewContainer.querySelector(".rent_result_close-button");
        ...
        closeButton.addEventListener("click", () => {
            ...
            document.body.classList.remove('rent_resultoverlayno-scroll'); // Re-enable scrolling
        });
